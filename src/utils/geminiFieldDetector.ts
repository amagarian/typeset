...
function buildPass1SharedSystemPrompt(): string {
  return [
    ...
    "  - **CRITICAL — NO LABEL-BLEEDING ACROSS GRID CELLS (v0.6.17):** When a form is laid out as a TABLE of cells (e.g. a 2×2 header `Date: / Company: / Contact: / Contract/Invoice #:`), each cell's `label` MUST come ONLY from the printed prefix INSIDE THAT SAME CELL. NEVER label a cell using the printed text from a NEIGHBOUR cell. A bare `Company:` cell's label is `Company`, NOT `Invoice #` because the cell to its right says `Contract/Invoice #:`. A bare `Contact:` cell's label is `Contact`, NOT `Accounting Contact` because some other contact-related text appears elsewhere on the page. Each cell is its own atomic field; do not infer a cell's label from its neighbours.",
    "    - Concrete example (Keslow CC Auth header grid):",
    "      ```",
    "      | Date:    ____________ | Contact:              ____________ |",
    "      | Company: ____________ | Contract/Invoice #:   ____________ |",
    "      ```",
    "      → emit FOUR fields with labels exactly: `Date`, `Contact`, `Company`, `Contract Invoice #`. Resolve `canonical_field_id` per cell from the catalog using THAT cell's label only. The Company cell's `canonical_field_id` is `productionCompany`, NOT a confused `Invoice` mapping.",
